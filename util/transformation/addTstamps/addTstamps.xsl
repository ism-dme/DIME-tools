...
<xsl:stylesheet exclude-result-prefixes="xs math xd mei uuid dme" version="3.0" xmlns:dme="http://www.mozarteum.at/ns/dme" xmlns:functx="http://www.functx.com" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:uuid="java:java.util.UUID" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xpath-default-namespace="http://www.music-encoding.org/ns/mei">
  <doc scope="stylesheet" xmlns="http://www.oxygenxml.com/ns/doc/xsl">
    <desc>
      ...
      <p><b>Contributors</b>: Johannes Kepper, Oleksii Sapov. <pre/>
    ...
    <xsl:for-each select=".//*">
      <xsl:choose>
        <xsl:when test="@sameas">
          ...
          <xsl:if test="$reference[(@dur and not(ancestor::chord) and not(@grace) and not(ancestor::graceGrp)) or (local-name() = $exceptions)]">
        ...
        <xsl:otherwise>
          <xsl:if test=".[(@dur and not(ancestor::chord) and not(@grace) and not(ancestor::graceGrp)) or (local-name() = $exceptions)]">
            <xsl:sequence select="."/>
          </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="layer//*" mode="events">
    ...
    <xsl:variable as="xs:boolean" name="constraints1" select="boolean(.[(@dur and not(ancestor::chord) and not(@grace) and not(ancestor::graceGrp)) or (local-name() = $exceptions)])"/>
    <xsl:variable as="xs:boolean" name="constraints2">
      <xsl:variable name="ref" select="id(substring-after(@sameas, '#'))"/>
      <xsl:sequence select="boolean($ref[(@dur and not(ancestor::chord) and not(@grace) and not(ancestor::graceGrp)) or (local-name() = $exceptions)])"/>
    </xsl:variable>
