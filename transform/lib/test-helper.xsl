...
<xsl:stylesheet exclude-result-prefixes="#all" version="3.0" xmlns:test-helper="x-urn:tutorial:helper:ws-only-text:test-helper" xmlns:worker="x-urn:tutorial:helper:ws-only-text:test-helper:worker" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">

  <!-- Disclaimer: Stylesheet originates from git@github.com:xspec/xspec.git-->
  <!--
  ...
  <!-- Identity template -->
  <xsl:template as="node()" match="attribute() | document-node() | node()" mode="worker:remove-whitespace-only-text">
    <xsl:copy>
  ...
  <xsl:template as="empty-sequence()" match="text()[normalize-space() => not()]" mode="worker:remove-whitespace-only-text"/>
