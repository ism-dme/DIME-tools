...
<schema queryBinding="xslt3" xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<pattern id="sameAs">
		<rule context="(mei:note | mei:rest | mei:mRest | mei:chord)[@sameas]">
			<let name="sameasVal" value="substring(@sameas, 2)"/>
			<let name="referencedElTstamp" value="id($sameasVal)/@tstamp"/>
			<assert role="error" test="@tstamp = $referencedElTstamp">The @tstamp of the elements (note, rest, mRest, chord) linked by @sameas should have the same value.</assert>
		</rule>
		<p>The @tstamps of the element linked by @sameas should have the same value.</p>

		<rule context="*[@sameas]">
			<let name="refName" value="substring-after(@sameas, '#') => id(.) => local-name()"/>
			<assert test="local-name() = $refName">The name of the element with @sameas and the name of the pointed element must match.</assert>
	...
	<pattern id="slur.staff">
		<rule context="mei:slur | mei:tie">
			<let name="startid.staff" value="id(substring(@startid, 2))/ancestor::mei:staff/@n"/>
			<let name="endid.staff" value="id(substring(@endid, 2))/ancestor::mei:staff/@n"/>
			<assert role="warning" test="(@staff = $startid.staff) and (@staff = $endid.staff)"> The linked notes by @startid or @endid should have the same @staff value as the slur|tie. </assert>
			<let name="startid.note" value="id(substring(@startid, 2))"/>
			<let name="endid.note" value="id(substring(@endid, 2))"/>
			<report role="warning" test="$startid.note/preceding::mei:note[@xml:id = $endid.note/@xml:id]">The note linked by @startid is placed before the note linked by @endid.</report>
		</rule>
		<p>The notes linked by @startid or @endid should have the same @staff value as the slur or tie.</p>
		<p>The note linked by @startid should not be placed before the note linked by @endid.</p>
	</pattern>
	...
	<!--/////-->
	<pattern id="clef">
		<rule context="mei:clef[not(@sameas)]">
			<assert role="warn" test="@shape and @line">&lt;clef> have @shape and @line</assert>
			<p>A &lt;clef> should have @shape and @line (except those having @sameas).</p>
		</rule>
	</pattern>
	<!--/////-->
	...
	<pattern id="work_identifier">
		<rule context="mei:work">
			<assert role="fatal" test="mei:identifier"> Work identifier tag is missing! </assert>
		</rule>
