...
--> 
	<xsl:function name="functx:remove-attributes" as="element()"
		...
		<xsl:for-each select="$elements">
<!--replaced xsl:element by xsl:copy as an empty xmlns was added
			||modified by Oleksii Sapov, 08.01.2020
			|| on the advice by Martin Honnen:
			|| https://stackoverflow.com/questions/59631924/functxremove-attributes-adds-an-empty-xmlns-->
			<xsl:copy>
				<xsl:sequence
